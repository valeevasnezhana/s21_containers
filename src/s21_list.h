...
class list {
 ...
 public:
  ...
  list(size_type n) : list() {
    if (n == 0) {
      throw std::out_of_range(
          "Index out of range");  //можно сделат ьпо другому сразу выделить
                                  //память для элементов
    }
  ...
  iterator insert(iterator pos, const_reference value) {
    ...
    // } else {
    //   Node *prev_node = const_cast<Node *>(static_cast<const Node *>(
    //       next_node->prev));  
  ...
  void unique() {
    if (size_ <= 1) {
      return;  // Нечего удалять, если список пустой или содержит только один
               // элемент
    }
  ...
  void sort() {
    if (size_ <= 1) {
      return;  // Нечего сортировать, если список пустой или содержит только
               // один элемент
    }
