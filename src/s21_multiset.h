...
class MultisetIterator {
 ...
 private:
  ...
  explicit MultisetIterator(typename s21::map<T, size_t>::iterator it)
      : iter(it), current_count_(1ul) {}
  MultisetIterator(typename s21::map<T, size_t>::iterator it, size_t count)
 ...
 public:
  ...
  multiset& operator=(multiset&& ms) noexcept {
    if (this != &ms) {
      ...
      size_ = std::move(ms.size_);
      ms.size_ = 0;
    }
    return *this;
  }

  iterator begin() const { return iterator(counter_.begin()); }

  iterator end() const { return iterator(counter_.end(), 1lu); }
  ...
  iterator find(const value_type& key) const {
    return iterator(counter_.find(key));
  }
